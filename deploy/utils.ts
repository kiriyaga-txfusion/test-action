...
import { Transaction, TransactionResponse } from "zksync2-js/src/types";
...
export const verifyEnoughBalance = async (wallet: Wallet, amount: BigNumberish) => {
  ...
  if (balance < toBigInt(amount)) throw `⛔️ Wallet balance is too low! Required ${formatEther(amount)} ETH, but current ${wallet.address} balance is ${formatEther(balance)} ETH`;
}
...
export async function wait(provider: Provider, transaction: TransactionResponse) {
